...
class PDDL_Parser:
    ...
    def parse_action(self, group):
        ...
        while group:
            ...
            if t == ':parameters':
                ...
                self.split_predicates(group.pop(0), add_effects, del_effects, name, ' effects', cond_effects)
                action_type = 'action'
            elif t == ':observe':
                self.split_predicates(group.pop(0), add_effects, del_effects, name, ' observe')
                action_type = 'observation'
            else: print(str(t) + ' is not recognized in action')
        a = Action(name, parameters, positive_preconditions, negative_preconditions, add_effects, del_effects, cond_effects)
    ...
    def split_predicates(self, group, pos, neg, name, part, cond=[]):
        if not type(group) is list:
        ...
        for predicate in group:
            ...
            elif predicate[0] == 'when':
                if len(predicate) != 3:
                    raise Exception('Unexpected when in ' + name + part)
                cond_pos = []
                cond_neg = []
                eff_pos = []
                eff_neg = []
                self.split_predicates(predicate[1], cond_pos, cond_neg, name, 'when')
                self.split_predicates(predicate[2], eff_pos, eff_neg, name, 'when')
                cond.append({'cond_pos': cond_pos, 'cond_neg': cond_neg,
