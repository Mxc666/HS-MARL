...
# Interpretative
SymOption_dict = {'0': 'r1AtStart, r2AtStart -> r1NearGoal, r2other', '1': 'r1AtStart, r2AtStart -> r1other, r2NearGoal', '2': 'r1AtStart, r2AtStart -> r1NearGoal, r2NearGoal',\
                  '3': 'r1NearGoal, r2other -> r1Goal, r2Goal', '4': 'r1other, r2NearGoal -> r1Goal, r2Goal', '5': 'r1NearGoal, r2NearGoal -> r1Goal, r2Goal'}
Action_SET = [[[0, 0, 1, 0], [0, 1, 0, 0]],  # r1NearGoal, r2other
             [[0, 1, 0, 0], [0, 0, 1, 0]],  # r1other r2NearGoal
             [[0, 0, 1, 0], [0, 0, 1, 0]]]  # r1NearGoal r2NearGoal
...
wandb_log = {}
cur_plan_so_list = []
reward_num = 0 # using to compute the average rewards
Steps = 0  # record the total step
maxStepsPerEpisode = args.episode_length   # Max step
episodes = int(args.episode_limit) // args.n_rollout_threads  # total episode/ threads
...
for episodeCount in range(episodes):  # record the episode, episodeCount is the current episode num
    ...
    turn_tag = False  # DataCollection
    another_option_num = 0  # successor node
    ...
    while (not envs.isTerminal()[0]) and episodeSteps <= maxStepsPerEpisode:
        ...
        Data_sets = []  # DataCollection store data
        ...
        while (not envs.isTerminal()[0]) and (DynaNumstate != S2numstate) and (episodeSteps <= maxStepsPerEpisode):
            ...
            # print('mappo_rewards'*10, mappo_rewards, mappo_rewards[0][0]) # mappo_rewards [[[0] [0]]] [0]
            ...
            #     Data_sets[-1][1] = [intrinsicRewards]  # 因为这次达到了其他action的goal, 所以赋予intrinsic rewards
        ...
        if chosen_shared_option <= 2  and (DynaNumstate == S2numstate):  # 选择的action的goal被达到了
    ...
    wandb_log['average_episode_rewards'] = cumulative_average_reward
    wandb_log['success_tracker'] = success_tracker
    if args.use_wandb:
        log_train(wandb_log, Steps)
